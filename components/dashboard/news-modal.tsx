...
import { addNews, updateNews } from '@/lib/features/dataSlice';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function NewsModal({ isOpen, onClose, news, mode }: NewsModalProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const newsData: News = {
      ...
      title: data.title,
      description: content,
      image: imageFiles.length > 0 ? URL.createObjectURL(imageFiles[0]) : news?.image,
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input
              id="title"
              {...register('title', { required: mode !== 'view' ? 'Title is required' : false })}
          ...
          {mode === 'view' && news && (
            <div className="space-y-4">
              <div>
                ...
                <div className="prose max-w-none border rounded-md p-4 bg-gray-50" dangerouslySetInnerHTML={{ __html: news.description }} />
              </div>
